...
impl<'a> PartialEq for Expression<'a> {
    fn eq(&self, other: &Expression<'a>) -> bool {
        self.ws == other.ws && comp_opt(&self.keyval, &other.keyval) && comp_opt(&self.table, &other.table) &&
...
impl<'a> Display for Expression<'a> {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match (&self.ws, &self.keyval, &self.table, &self.comment) {
            ...
            _ => {
                panic!("Invalid expression: ws1: \"{}\", ws2: \"{}\", keyval: {:?}, table: {:?}, comment: {:?}",
                       self.ws.ws1,
                       self.ws.ws2,
                       self.keyval,
                       self.table,
...
impl<'a> PartialEq for KeyVal<'a> {
    fn eq(&self, other: &KeyVal<'a>) -> bool {
        self.key == other.key && self.keyval_sep == other.keyval_sep && self.val == other.val
    }
}
...
impl<'a> PartialEq for CommentNewLines<'a> {
    fn eq(&self, other: &CommentNewLines<'a>) -> bool {
        self.pre_ws_nl == other.pre_ws_nl && self.comment == other.comment && self.newlines == other.newlines
    }
}
...
impl<'a> CommentNewLines<'a> {
    pub fn new_str(pre_ws_nl: &'a str, comment: Comment<'a>, newlines: &'a str) -> CommentNewLines<'a> {
        CommentNewLines {
            pre_ws_nl: pre_ws_nl.into(),
            comment: comment,
            newlines: newlines.into(),
        }
    }
    pub fn new_string(pre_ws_nl: String, comment: Comment<'a>, newlines: String) -> CommentNewLines<'a> {
        CommentNewLines {
            pre_ws_nl: pre_ws_nl.into(),
            comment: comment,
            newlines: newlines.into(),
...
impl<'a> TimeOffsetAmount<'a> {
    pub fn new_str(pos_neg: &'a str, hour: &'a str, minute: &'a str) -> TimeOffsetAmount<'a> {
        let pn = match pos_neg {
            ...
            "-" => PosNeg::Neg,
            _ => {
                error!("PosNeg value is neither a '+' or a '-', defaulting to '+'.");
                PosNeg::Pos
            },
        };
        TimeOffsetAmount {
...
impl<'a> PartialEq for ArrayValue<'a> {
    fn eq(&self, other: &ArrayValue<'a>) -> bool {
        self.val == other.val && comp_opt(&self.array_sep, &other.array_sep) && self.comment_nls == other.comment_nls
    }
}
...
impl<'a> ArrayValue<'a> {
    pub fn new(val: Rc<RefCell<TOMLValue<'a>>>, array_sep: Option<WSSep<'a>>, comment_nls: Vec<CommentOrNewLines<'a>>)
               -> ArrayValue<'a> {
        ArrayValue {
            val: val,
            array_sep: array_sep,
...
impl<'a> PartialEq for Array<'a> {
    fn eq(&self, other: &Array<'a>) -> bool {
        self.values == other.values && self.comment_nls1 == other.comment_nls1 &&
...
impl<'a> Array<'a> {
    pub fn new(values: Vec<ArrayValue<'a>>, comment_nls1: Vec<CommentOrNewLines<'a>>,
               comment_nls2: Vec<CommentOrNewLines<'a>>)
               -> Array<'a> {
        Array {
            values: values,
            comment_nls1: comment_nls1,
            comment_nls2: comment_nls2,
        }
    }
}
...
impl<'a> PartialEq for TableKeyVal<'a> {
    fn eq(&self, other: &TableKeyVal<'a>) -> bool {
        self.keyval == other.keyval && self.kv_sep == other.kv_sep && self.comment_nls == self.comment_nls
    }
}
...
impl<'a> TableKeyVal<'a> {
    ...
    pub fn default<S>(key: S, val: Rc<RefCell<TOMLValue<'a>>>) -> TableKeyVal<'a>
        where S: Into<String>
    {
        let keyval = KeyVal::new_string(key.into(), WSSep::new_str(" ", " "), val);
        TableKeyVal {
            keyval: keyval,
            kv_sep: Some(WSSep::new_str("", " ")),
            comment_nls: vec![],
        }
    }
    pub fn last<S>(key: S, val: Rc<RefCell<TOMLValue<'a>>>) -> TableKeyVal<'a>
        where S: Into<String>
    {
        let keyval = KeyVal::new_string(key.into(), WSSep::new_str(" ", " "), val);
        TableKeyVal {
...
impl<'a> PartialEq for InlineTable<'a> {
    fn eq(&self, other: &InlineTable<'a>) -> bool {
        self.keyvals == other.keyvals && self.ws == other.ws
    }
}
        ...
        let _toml = Toml {
            exprs: vec![NLExpression {
                            ...
                            expr: Expression {
                                ...
                                keyval: None,
                                table: None,
                                comment: Some(Comment { text: "hello, this is a comment".into() }),
                            },
                        }],
        };
    }
