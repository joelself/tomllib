...
impl<'a> Parser<'a> {
    ...
    pub fn get_value<S>(self: &Parser<'a>, key: S) -> Option<Value<'a>>
        where S: Into<String>
    {
        let s_key = key.into();
    ...
    pub fn set_value<S>(self: &mut Parser<'a>, key: S, val: Value<'a>) -> bool
        where S: Into<String>
    {
        let s_key = key.into();
        {
        ...
        let new_value_clone = match new_value {
            TOMLValue::Array(ref rc_rc) => TOMLValue::Array(rc_rc.clone()),
            TOMLValue::InlineTable(ref rc_rc) => TOMLValue::InlineTable(rc_rc.clone()),
            unknown => {
                panic!("In Parser.set_value, new_value should only be an Array or InlineTable. Instead it's a {:?}",
                       unknown)
            },
        };
        if self.map.contains_key(&s_key) {
            ...
            let val_rf = match opt_value {
                &mut Some(ref mut v) => v,
                &mut None => {
                    panic!("existing_value's value is None, when this should've returned false earlier in the \
                            function.")
                },
            };
    ...
    fn convert_vector(tval: &Value<'a>) -> Option<TOMLValue<'a>> {
        ...
        match tval {
            &Value::Array(ref arr) => {
                ...
                return Some(TOMLValue::Array(Rc::new(RefCell::new(Array::new(values, vec![], vec![])))));
            },
            &Value::InlineTable(ref it) => {
                ...
                return Some(TOMLValue::InlineTable(Rc::new(RefCell::new(InlineTable::new(key_values,
                                                                                         WSSep::new_str(" ", " "))))));
            },
            &Value::Integer(ref s) => {
                if tval.validate() {
    ...
    fn rebuild_vector(self: &mut Parser<'a>, key: String, val: Rc<RefCell<TOMLValue<'a>>>, skip: bool) {
        match *val.borrow() {
            ...
            TOMLValue::InlineTable(ref it) => {
                {
                    let value = match self.map.entry(key.clone()) {
                        Entry::Occupied(entry) => entry.into_mut(),
                        _ => {
                            panic!("Map contains inline table key, but map.entry returned a Vacant entry is set_value.")
                        },
                    };
                    if !skip {
                        value.value = Some(val.clone());
            ...
            _ => {
                self.map.entry(key.clone()).or_insert(HashValue::new_count(val.clone()));
            },
        }
    }
    ...
    fn replace_values(val_rf: &Rc<RefCell<TOMLValue<'a>>>, tval: &Value<'a>) -> bool {
        let value = match (&*val_rf.borrow(), tval) {
            ...
            (v, tv) => {
                panic!("Check for the same structure should have eliminated the possibility of replacing {} with {}",
                       v,
                       tv)
            },
        };
...
"#;
    ...
    fn test_key_inline_table() {
        ...
        assert_eq!(p.get_value("car.properties.accident_dates[2]"),
                   res2opt!(Value::datetime_from_str("2014", "11", "30", "03", "13", "54")));
    }
    ...
    fn test_get_root_children() {
        ...
        let p = Parser::new();
        let (p, _) = p.parse(TT::get());
        assert_eq!(p.get_children(""),
                   Some(&Children::Keys(RefCell::new(vec!["animal".to_string(), "car".to_string()]))));
    }
    ...
    fn test_get_inline_table_children() {
        ...
        assert_eq!(p.get_children("car.properties"),
                   Some(&Children::Keys(RefCell::new(vec!["color".to_string(),
                                                          "\"plate number\"".to_string(),
                                                          "accident_dates".to_string()]))));
    }
    ...
    fn test_set_bare_key() {
        ...
        assert_eq!(p.get_value("animal"), Some(Value::String("shark".into(), StrType::MLBasic)));
    }
    ...
    fn test_set_table_key() {
        ...
        assert_eq!(p.get_value("car.model"), Some(Value::String("Accord".into(), StrType::Literal)));
    }
    ...
    fn test_set_array_element_key() {
        ...
        assert_eq!(p.get_value("car.drivers[1]"), Some(Value::String("Mark".into(), StrType::MLLiteral)));
    }
    ...
    fn test_set_nested_aray_element_key() {
        ...
        assert_eq!(p.get_value("car.properties.accident_dates[2]"), Some(Value::Float("3443.34".into())));
    }
    ...
    fn test_set_inline_table_element_key() {
        ...
        assert_eq!(p.get_value("car.properties.color"), Some(Value::Integer("19".into())));
    }
    ...
    fn test_truncate_array() {
        ...
        let p = Parser::new();
        let (mut p, _) = p.parse(TT::get());
        p.set_value("car.drivers",
                    Value::Array(Rc::new(vec![Value::basic_string("Phil").unwrap(),
                                              Value::basic_string("Mary").unwrap()])));
        assert_eq!(p.get_value("car.drivers"),
                   Some(Value::Array(Rc::new(vec![Value::String("Phil".into(), StrType::Basic),
    ...
    fn test_truncate_inline_table() {
        ...
        let p = Parser::new();
        let (mut p, _) = p.parse(TT::get());
        p.set_value("car.properties",
                    Value::InlineTable(Rc::new(vec![("make".into(), Value::literal_string("Honda").unwrap()),
                                                    ("transmission".into(), Value::bool(true))])));
        assert_eq!(p.get_value("car.properties"),
                   Some(Value::InlineTable(Rc::new(vec![("make".into(),
                                                         Value::String("Honda".into(), StrType::Literal)),
    ...
    fn test_extend_array() {
        ...
        p.set_value("car.drivers",
                    Value::Array(Rc::new(vec![Value::int(1),
                                              ...
                                              Value::int(8)])));
        assert_eq!(p.get_value("car.drivers"),
                   Some(Value::Array(Rc::new(vec![Value::Integer("1".into()),
    ...
    fn test_extend_inline_table() {
        ...
        let (mut p, _) = p.parse(TT::get());
        p.set_value("car.properties",
                    Value::InlineTable(Rc::new(vec![("prop1".into(), Value::bool_from_str("TrUe").unwrap()),
        ...
        assert_eq!(p.get_value("car.properties"),
                   Some(Value::InlineTable(Rc::new(vec![("prop1".into(), Value::Boolean(true)),
    ...
    fn test_set_implicit_table_key() {
        ...
        assert_eq!(p.get_value("car.interior.seats.type"), Some(Value::String("leather".into(), StrType::Basic)));
    }
    ...
    fn test_set_array_of_table0_key() {
        ...
        assert_eq!(p.get_value("car.owners[0].Age"), Some(Value::Float("19.5".into())));
    }
    ...
    fn test_set_array_of_table1_key() {
        ...
        assert_eq!(p.get_value("car.owners[1].Name"), Some(Value::String("Steve Parker".into(), StrType::MLBasic)));
    }
