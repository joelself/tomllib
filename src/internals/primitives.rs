...
impl<'a> Parser<'a> {
    pub fn get_key_parent(tables: &RefCell<Vec<Rc<TableType<'a>>>>, tables_index: &RefCell<Vec<usize>>) -> String {
    ...
    pub fn get_array_table_key(map: &RefCell<&mut HashMap<String, HashValue<'a>>>,
                                ...
                                match hash_value.subkeys {
                                    ...
                                    Children::Keys(ref hs_rf) => {
                                        if hs_rf.borrow().contains(&keys[j + 1]) || j + 1 == len - 1 {
                                            ...
                                            panic!("Key does not exist in map: \"{}\"",
                                                   &format!("{}.{}", full_key, &keys[j + 1]));
                                        }
                                    },
                                }
                            },
    ...
    pub fn get_full_key(map: &RefCell<&mut HashMap<String, HashValue<'a>>>, tables: &RefCell<Vec<Rc<TableType<'a>>>>,
                        tables_index: &RefCell<Vec<usize>>, keychain: &RefCell<Vec<Key<'a>>>)
                        -> (bool, String, String) {

        let (valid, array_key, _) = Parser::get_array_table_key(map, tables, tables_index);
    ...
    pub fn insert_keyval_into_map(&mut self, val: Rc<RefCell<TOMLValue<'a>>>) {
            ...
            &Some(ref ttype) => {
                match **ttype {
                    TableType::Standard(_) => {
                        ...
                        let tuple = Parser::get_full_key(&map,
                                                         &self.last_array_tables,
        ...
        if error {
            ...
            self.errors
                .borrow_mut()
                .push(ParseError::DuplicateKey(full_key, self.line_count.get(), 0, to_val!(&*val.borrow())));
        } else if setvalue || insert {
            if setvalue {
                debug!("Set existing hash value. full_key: {}, parent_key: {}, val: {}",
                       full_key,
                       parent_key,
                       *(*val).borrow());
                ...
                match *val.borrow() {
                    TOMLValue::InlineTable(_) => {
                        map.borrow_mut().insert(full_key.clone(), HashValue::new_keys(val.clone()))
                    },
    ...
    // Integer
    method!(pub integer<Parser<'a>, &'a str, &'a str>, self, re_find!(r#"^((\+|-)?(([1-9](\d|(_\d))+)|\d))"#)) ;
   ...
   string: re_find!(r#"^'''(	|[ -􏿿]|\n|(\r\n))*?'''"#),
    ...
    fn ml_basic_string(mut self: Parser<'a>, input: &'a str) -> (Parser<'a>, nom::IResult<&'a str, &'a str>) {
        ...
        let r = match raw {
            ...
            IResult::Error(_) => {
                IResult::Error(nom::Err::Code(nom::ErrorKind::Custom(ErrorCode::MLLiteralString as u32)))
            },
    ...
    method!(string<Parser<'a>, &'a str, TOMLValue>, mut self,
    alt!(
    ...
    // Boolean
    method!(boolean<Parser<'a>, &'a str, bool>, self, alt!(complete!(tag_s!("false")) => {|_| false} |
 ...
 fraction: complete!(call_m!(self.fractional)) ?  ~
  ...
  pos_neg: alt!(complete!(tag_s!("+")) | complete!(tag_s!("-")))  ~
   ...
   minute: re_find!("^[0-9]{2}")                                  ,
    ...
    pub fn date_time(mut self: Parser<'a>, input: &'a str) -> (Parser<'a>, IResult<&'a str, DateTime>) {
        ...
        match result {
            IResult::Done(i, o) => {
                if !o.validate() {
                    ...
                    self.errors
                        .borrow_mut()
                        .push(ParseError::InvalidDateTime(Parser::get_full_key(&RefCell::new(&mut self.map),
                                                                               &self.last_array_tables,
                                                                               &self.last_array_tables_index,
                                                                               &self.keychain)
                                                              .1,
    ...
    method!(pub val<Parser<'a>, &'a str, Rc<RefCell<TOMLValue>> >, mut self,
    alt!(
...
Â£Ã¯Ã±Ã¨ TÎ»Å™Ã¨Ã¨"#));
    ...
    fn test_string() {
        ...
        assert_eq!(p.string("\"Î²Ã¡Æ¨Ã¯Ã§_Æ¨Æ­Å™Ã¯Ã±Ï±\"").1,
                   Done("",
                        TOMLValue::String("Î²Ã¡Æ¨Ã¯Ã§_Æ¨Æ­Å™Ã¯Ã±Ï±".into(), StrType::Basic)));
        p = Parser::new();
...
""""#)
                    .1,
                   Done("",
                        TOMLValue::String(r#"â‚¥â„“_Î²Ã¡Æ¨Ã¯Ã§_Æ¨Æ­Å™Ã¯Ã±Ï±
Ã±Ãºâ‚¥Î²Ã¨Å™_Æ­Ï‰Ã´
NÃ›MÃŸÃ‰R-THRÃ‰Ã‰
"#
                                              .into(),
...
'''"#)
                    .1,
                   Done("",
                        TOMLValue::String(r#"Â§Æ¥Å™Ã¯Æ­Ã¨
Ã‡Ã´Æ™Ã¨
ÃžÃ¨Æ¥Æ¨Ã¯
"#
                                              .into(),
    ...
    fn test_time() {
        let mut p = Parser::new();
        assert_eq!(p.time("T11:22:33.456").1, Done("", Time::new_str("11", "22", "33", Some("456"), None)));
        p = Parser::new();
        assert_eq!(p.time("T04:05:06").1, Done("", Time::new_str("04", "05", "06", None, None)));
    }

    #[test]
    fn test_time_offset_amount() {
        let p = Parser::new();
        assert_eq!(p.time_offset_amount("+12:34").1, Done("", TimeOffsetAmount::new_str("+", "12", "34")));
    }

    #[test]
    fn test_time_offset() {
        let mut p = Parser::new();
        assert_eq!(p.time_offset("+12:34").1, Done("", TimeOffset::Time(TimeOffsetAmount::new_str("+", "12", "34"))));
        p = Parser::new();
    ...
    fn test_full_date() {
        let p = Parser::new();
        assert_eq!(p.date("1942-12-07").1, Done("", Date::new_str("1942", "12", "07")));
    }
    ...
    fn test_date_time() {
        ...
        assert_eq!(p.date_time("1999-03-21T20:15:44.5-07:00").1,
                   Done("",
                        DateTime::new(Date::new_str("1999", "03", "21"),
                                      Some(Time::new_str("20",
                                                         "15",
                                                         "44",
                                                         Some("5"),
    ...
    fn test_quoted_key() {
        let p = Parser::new();
        assert_eq!(p.quoted_key("\"QÃºÃ´Æ­Ã¨Î´KÃ¨Â¥\"").1,
                   Done("", "\"QÃºÃ´Æ­Ã¨Î´KÃ¨Â¥\""));
    }

    #[test]
    fn test_key() {
        let mut p = Parser::new();
        assert_eq!(p.key("\"GÅ™Ã¡Æ¥Ã¨Æ’Å™ÃºÃ¯Æ­\"").1,
                   Done("", "\"GÅ™Ã¡Æ¥Ã¨Æ’Å™ÃºÃ¯Æ­\""));
        p = Parser::new();
    ...
    fn test_val() {
      ...
      DateTime::new(Date::new_str("2112", "09", "30"), Some(Time::new_str("12", "33", "01", Some("345"),
        ...
        p = Parser::new();
        assert_eq!(p.val("'Â§Ã´â‚¥Ã¨ Â§Æ­Å™Ã¯Ã±Ï±'").1,
                   Done("",
                        Rc::new(RefCell::new(TOMLValue::String("Â§Ã´â‚¥Ã¨ Â§Æ­Å™Ã¯Ã±Ï±"
                                                                   .into(),
                                                               StrType::Literal)))));
    }
    ...
    fn test_keyval() {
        let p = Parser::new();
        assert_eq!(p.keyval("Boolean = 84.67").1,
                   Done("",
                        KeyVal::new_str("Boolean",
                                        WSSep::new_str(" ", " "),
